...
type Step = 'otp' | 'password' | 'complete';
...
export default function PortalInvitePage() {
    ...
    return (
      <div style={pageStyle}>
        <div style={cardStyle}>
          ...
          <button onClick={handleVerifyOtp} disabled={otp.join('').length !== 6 || isSubmitting}
            style={{
              width: '100%', padding: '14px', backgroundColor: otp.join('').length === 6 ? '#016991' : '#94a3b8',
              color: 'white', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: 600,
              cursor: otp.join('').length === 6 ? 'pointer' : 'not-allowed', marginBottom: '16px',
            }}>
            {isSubmitting ? 'Verifying...' : 'Verify & Continue'}
          </button>
          <p style={{ textAlign: 'center', color: '#64748b', fontSize: '14px', margin: 0 }}>
            Didn't receive a code?{' '}
            <button style={{ background: 'none', border: 'none', color: '#016991', cursor: 'pointer', fontWeight: 500, padding: 0 }}>
              Resend
            </button>
          </p>
        </div>
  ...
  if (step === 'password') {
    return (
      <div style={pageStyle}>
        <div style={cardStyle}>
          {logoBlock}
          <h1 style={{ textAlign: 'center', fontSize: '22px', fontWeight: 600, color: '#0f172a', margin: '0 0 8px 0' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, color: '#334155', marginBottom: '6px' }}>Username</label>
            <input type="text" value={clientData.email} readOnly
              style={{ width: '100%', padding: '12px 16px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '15px', backgroundColor: '#f8fafc', color: '#64748b', boxSizing: 'border-box' }} />
            <p style={{ fontSize: '13px', color: '#016991', margin: '6px 0 0 0' }}>This is your login for the Wealth Portal</p>
          </div>

          {/* Password */}
          <div style={{ marginBottom: '16px' }}>
            <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, color: '#334155', marginBottom: '6px' }}>Create Password</label>
            <div style={{ position: 'relative' }}>
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)}
                placeholder="Enter your password"
                style={{ width: '100%', padding: '12px 44px 12px 16px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '15px', boxSizing: 'border-box' }} />
          ...
          <button onClick={handleCreateAccount} disabled={!allRequirementsMet || isSubmitting}
            style={{
  ...
  return (
    <div style={pageStyle}>
      <div style={cardStyle}>
        <div style={{ width: '64px', height: '64px', borderRadius: '50%', backgroundColor: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
          <span className="material-icons" style={{ fontSize: '32px', color: '#16a34a' }}>check</span>
        </div>
        <h1 style={{ textAlign: 'center', fontSize: '24px', fontWeight: 600, color: '#0f172a', margin: '0 0 8px 0' }}>You're All Set!</h1>
        <p style={{ textAlign: 'center', color: '#64748b', margin: '0 0 32px 0', fontSize: '15px' }}>
          Your Wealth Portal account is ready. Access your portfolio anytime, anywhere.
        </p>

        <div style={{ backgroundColor: '#f8fafc', borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
          <p style={{ fontSize: '12px', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 12px 0' }}>Your Login Details</p>
        ...
        <button onClick={() => alert('This would open the Wealth Portal')}
          style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
          <a href="#" style={{ display: 'block', backgroundColor: '#000', borderRadius: '8px', padding: '8px 16px', color: 'white', textDecoration: 'none' }}>
        ...
        <div style={{ marginTop: '24px', textAlign: 'center' }}>
          <a href="/comms-hub" style={{ color: '#64748b', fontSize: '14px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
